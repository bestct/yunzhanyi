...
createApp({
    created() {
        ...
        this.indexTag = document.getElementById('indexTag').value
        instance({
                url: "/tag/" + this.indexTag,
                method: 'get'
            }
        ).then(res => {
    ...
    methods: {
        getData() {
            this.loading = true
            instance({
                    url: "/index/poetry?dynasty=" + this.indexDynasty +"&tag="+this.indexTag +"&pageNum=" + this.pageIndex + "&pageSize=" + this.pageSize,
                    method: 'get'
                }
            ).then(res => {
